...
function StackedGraph({ dataList }: WealthProps) {
    ...
    const graphData = addEmptyDates(dataList)
        ...
        .map((item) => ({
            id: item.id,
            data: sequence.map((i) => item.data[Math.floor(((i + 1) * item.data.length) / numberOfPoints - 1)]),
        }));
        ...
        <AutoSizer>
            {({ height, width }) => (
                <div style={{ height, width: width }}>
                    <ResponsiveLine
                        margin={{ right: 48, top: 4, bottom: 150 }}
                        data={graphData}
                        xScale={{ type: "time" }}
                        yScale={{
                            type: "linear",
